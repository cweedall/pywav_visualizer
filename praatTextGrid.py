...
class PraatTextGrid:
	...
	def readFromFile(self, fileName):
		...
		if encoding['encoding'] == 'ASCII' or encoding['encoding'] == 'UTF-8':
			...
			decoded_text = encoded_text.decode(encoding='UTF-16')
		elif encoding['encoding'] == 'UTF-16BE':
			bom = codecs.BOM_UTF16_BE
			if encoded_text.startswith(bom):
				encoded_text = encoded_text[len(bom):]#strip away the BOM
			decoded_text = encoded_text.decode(encoding='UTF-16BE')
		elif encoding['encoding'] == 'UTF-16LE':
			bom = codecs.BOM_UTF16_LE
			if encoded_text.startswith(bom):
				encoded_text = encoded_text[len(bom):]#strip away the BOM
			decoded_text = encoded_text.decode(encoding='UTF-16LE')
		else:
			raise Exception("File '"+str(fileName).replace("\\", "/")+"' encoding is '"+str(encoding['encoding'])+"' and cannot be processed")
		...
		for line in decoded_text.splitlines():
			txt = str(line)
			cnt += 1
			errMsg = 'readPraatAbstractTier(' + str(fileName) + ' - line ' + str(cnt) + ' [' + str(txt) + ']) - '
